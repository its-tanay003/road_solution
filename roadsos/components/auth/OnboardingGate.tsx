...
type AuthProviderMap = Record<string, { id: string; name: string }>;
...
export function OnboardingGate({ children }: { children: React.ReactNode }) {
  ...
  const [authProviders, setAuthProviders] = useState<AuthProviderMap | null>(null);
  ...
  useEffect(() => {
    ...
    getProviders()
      .then((providers) => {
        if (mounted) setAuthProviders((providers || {}) as AuthProviderMap);
      })
      .catch(() => {
        if (mounted) setAuthProviders({});
      });
  ...
  const hasGoogle = Boolean(authProviders?.google);
  const hasApple = Boolean(authProviders?.apple);
    ...
    return (
      <div className="min-h-screen w-full bg-gray-950 flex flex-col justify-center items-center p-4 relative overflow-y-auto">
        ...
        >
          ...
          <p className="text-gray-400 text-sm max-w-xs mb-6">Emergency Intelligence. Everywhere.</p>
          ...
          <div className="w-full space-y-3.5">
            ...
            {authProviders && !hasGoogle && !hasApple && (
              <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-left text-sm font-medium text-amber-100">
                Authentication providers are not configured for this environment.
              </div>
            )}
          </div>

          {authProviders === null && (
            <div className="mt-5 flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-500">
              <div className="h-3 w-3 rounded-full border-2 border-gray-600 border-t-transparent animate-spin" />
