    ...
    <div className="flex flex-col min-h-screen bg-(--nx-bg-base) text-(--nx-text-primary)">
      ...
      <div className="h-64 bg-(--nx-bg-[var(--color-surface))] relative border-b border-(--nx-border) overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
           <div className="w-32 h-32 bg-(--nx-red-primary)/10 rounded-full blur-[60px]" />
      ...
      {/* Content Area */}
      <div className="flex-1 p-6 lg:p-10 -mt-10 bg-(--nx-bg-base) rounded-t-[40px] relative z-20 border-t border-(--nx-border) shadow-[0_-20px_40px_rgba(0,0,0,0.8)]">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-4 text-(--nx-text-dim) uppercase font-mono text-xs">
             ...
             <div className="lg:col-span-5 space-y-6">
                <div className="flex flex-col gap-4">
                   <button 
                     ...
                     className="w-full h-16 bg-(--nx-green-primary) text-navy font-black text-lg uppercase tracking-[0.3em] flex items-center justify-center gap-4 hover:brightness-110 transition-all shadow-[0_0_30px_rgba(46,196,182,0.3)] group relative overflow-hidden"
                   >
                      <Phone size={24} />
                      INITIATE COMS
                ...
                <div className="p-4 nexus-card bg-(--nx-red-dim) border-(--nx-red-primary)/20 flex gap-4">
                   <AlertTriangle className="text-(--nx-red-primary) shrink-0" size={20} />
                   <div>
                      <div className="text-[10px] font-bold text-(--nx-red-primary) uppercase mb-1">INTEL INTEGRITY</div>
...
const MetricBox = ({ label, value, icon, accent }: any) => (
  <div className={`p-6 nexus-card flex flex-col gap-4 ${accent ? 'border-(--nx-red-primary)/20 bg-(--nx-red-dim)/30' : 'bg-white/[0.02]'}`}>
     ...
     <div className={`text-3xl font-black font-mono tracking-tighter ${accent ? 'text-(--nx-red-primary) shadow-[0_0_20px_rgba(255,59,59,0.1)]' : 'text-white'}`}>{value}</div>
  </div>
);
