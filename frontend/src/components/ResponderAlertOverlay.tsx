...
export const ResponderAlertOverlay = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="w-full max-w-sm bg-navy border-2 border-(--color-emergency)/50 rounded-2xl shadow-[0_0_50px_rgba(230,57,70,0.3)] overflow-hidden">
        
        {/* Pulsing Header */}
        <div className="bg-[rgba(255,59,59,0.20)] border-b border-(--color-emergency)/30 p-4 flex items-center justify-center space-x-3">
          <AlertTriangle className="text-(--color-emergency) animate-pulse" size={24} />
        ...
        <div className="p-6 space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted">User: {(activeAlert.user as { name?: string })?.name || 'Unknown'}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            ...
            <button 
              ...
              className="py-3 px-4 rounded-xl font-bold flex items-center justify-center bg-(--color-emergency) text-white hover:bg-emergency/80 transition-colors shadow-lg"
            >
